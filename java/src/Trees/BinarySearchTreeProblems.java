...
Explanation 2:
 ...
 The inorder traversal of binary tree is: [8, 10, 11]. Return the head pointer of the circular doubly linked list.
    */
    ...
    /* Two Sum BST : check if there exist a pair of nodes with given sum
...
Explanation 2:
 ...
 No such pair exists.
    */
    public boolean findPairUsingBSTIterator(TreeNode root, int sum) {
        Stack<TreeNode> forwardIterator = new Stack<TreeNode>();

        Stack<TreeNode> backwardIterator = new Stack<TreeNode>();

        TreeNode curr = root;
        while (curr != null) {
            forwardIterator.push(curr);
            curr = curr.left;
        }

        curr = root.right;
        while (curr != null) {
            backwardIterator.push(curr);
            curr = curr.right;
        }

        while (!forwardIterator.isEmpty() && !backwardIterator.isEmpty()) {
            int x = forwardIterator.peek().val;
            int y = backwardIterator.peek().val;
            int currSum = x + y;
            ...
            if (currSum > sum) {
                TreeNode node = backwardIterator.pop();
                if (backwardIterator.isEmpty())
                    return false;
                ...
                TreeNode node = forwardIterator.pop();
                if (forwardIterator.isEmpty())
                    return false;
